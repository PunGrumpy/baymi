...
agent/
  ...
  lib/                      # the only place logic lives; every module has a colocated *.test.ts
    ...
    github/
      ...
      pull-requests.ts      # which pull request events start a review, what a follow-up after a push is told, how a review session is recognized, the PR a session is anchored to
      ...
      anchors.ts            # settles each finding on a line of the diff before posting: no line → first added line, wrong line → nearest, untouched file → body
      checks.ts             # the check run on the head commit: in progress at dispatch, success, neutral or failure when the review settles
...
| --- | --- | --- | --- |
| Pull request review | `agent/channels/github.ts` + `agent/lib/github/pull-requests.ts` + `agent/lib/github/review.ts` + `agent/instructions/review.ts` + the `security-review` skill | Channel hook | `onPullRequest` dispatches on `opened`, `ready_for_review` and `synchronize`, skipping drafts and bots (`shouldReviewPullRequest`). A push continues the session the first review ran in, and `followUpContext` tells the model the turn is a follow-up: it reads its earlier findings with `listPullRequestReviewThreads`, posts only findings that have no thread yet, and says in the first line which earlier ones the push fixed and which are still open, since the turn cannot reply in those threads. The session runs under the constructed reviewer principal `github:baymiai` (`agent/lib/trust.ts`), which every gate recognizes. GitHub writes are refused, memory is off, and the reply is the only output the turn may produce. eve puts the diff in context and checks the head commit out into the sandbox before the first model call. The skill holds the procedure and the format. The channel's `message.completed` handler parses the reply (`parseReview`), reads the diff back from `GET /pulls/{number}/files` and settles each finding on a line the diff contains (`anchorReview`: the file's first added line when the model named no line, the nearest changed line when it named one outside the diff), then submits it as one GitHub review with `event: COMMENT`: each finding becomes an inline comment on that line, with one visible sentence, a `<details>` block for the reasoning, an optional `suggestion` block, and a hidden `<!-- baymi:finding -->` marker; the body is the count line and nothing else, whatever the model wrote there, so the review is the inline comments, as Vercel Agent posts. A clean review is one line plus at most one sentence, posted as a comment. A finding on a file the change does not touch goes into the body with its severity and path. A reply that does not parse, or a review GitHub rejects, is posted as an ordinary comment. A review that fails posts nothing on the pull request and posts to Slack instead, so a missing review is not mistaken for a clean one. A review that answered without a settled read beyond the skill load, or whose reply does not parse as a review, takes that same route (`agent/lib/github/grounding.ts`). The gateway's tool-call emulation can deliver the model's call as prose, and a turn that never reached the skill or the checkout has read only the truncated diff, so it must not be able to call the change clean. A check run named `Baymi security review` (`agent/lib/github/checks.ts`) opens on the head commit at dispatch and settles with the review: `success` for nothing to raise, `neutral` for findings, because the review reports and never gates a merge, and `failure` when no review reached the pull request, which is the author's only sign that the change went unread. It is opened from `onPullRequest` rather than a `turn.started` handler, because a handler supplied in `events` replaces eve's built-in, and the built-in `turn.started` is what checks the repository out. The run is looked up by name when it settles rather than carried in memory, since the two events can land in different invocations |
...
1. **A pull request opens** on an installed repository. GitHub posts `pull_request` at `/eve/v1/github`; `shouldReviewPullRequest` accepts `opened`, `ready_for_review` and `synchronize` from a human, the channel opens the check run on the head commit, and it starts a session under the reviewer principal. eve injects the diff and checks the head commit out. The agent loads `security-review`, reads the diff and the surrounding code, and writes its reply in the skill's format. The channel submits that reply as a GitHub review: one inline comment per finding, on its line, and the summary as the review body. If a finding is critical or high it calls `notify_maintainer` once, and the maintainer gets one line in Slack with a link. Every later push runs the review again on the new head, with a new check run, and posts only what is new.
