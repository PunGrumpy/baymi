...
/**
 * Weekly read of what the agent ran, from its own wide events.
 *
 * @remarks
 * Mondays at 10:00 UTC, an hour after the digest, so the week opens with the
 * issues and then with what last week took. The procedure lives in the
...
const run: ScheduleRunHandler = maintenanceRun(
  ...
  "Load the cost-watchdog skill and review the last full week: how many turns, how many tokens, and what changed against the week before. Say plainly when a number is missing rather than filling the gap."
);
