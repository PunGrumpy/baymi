...
Before adding a capability (tool, skill, memory, instruction fragment, channel hook), read `docs/capability-placement.md`. It decides where the capability lives and holds the two-layer rule: files under `agent/` are wiring, logic goes in `agent/lib/` with a colocated test. `docs/notes.md` collects the runtime and tooling behaviour that cost time to discover; read it before debugging something that looks impossible. `ARCHITECTURE.md` is the map of what already exists, including why the agent is read-only: it has no shell, writes no files, and fetches no URLs, and that is a decision, not a gap.
