...
**The `before-and-after` CLI is frozen upstream; the pin is a choice, not a lag.** On 2026-08-05 vercel-labs reduced the npm package to a “frozen alias” and made `SKILL.md` plus a dependency-free `format.mjs` the product. In that design `agent-browser` takes the frames, `gh --attach` uploads them, and the script formats a `<!-- before-and-after:start/end -->` block and replaces it in the body. `0.0.4` still captures a pair and still prints its `Saved:` lines, so the pair path keeps it. The single-frame path in `agent/lib/capture.ts` (`previewCommand`) already runs `agent-browser` directly, the way the CLI does internally, which is the path a move off the CLI would take. The markers match upstream’s, so the blocks are interchangeable.

**`gh --attach` refuses an installation token, so it is not a way for this agent to host a screenshot.** GitHub CLI 2.99.0 (2026-09-01) added `--attach` to `pr comment`, `pr create`, `pr edit` and the `issue` equivalents. It uploads through `uploads.github.com/user-attachments/assets?repository_id=…` and needs write access on the repository; upstream’s own CI notes that the endpoint answers an Actions installation token with `unsupported authentication type` and runs the live job on a user PAT instead. Every GitHub credential this agent holds is an installation token from Connect, and `git_push` exists to keep even that out of the sandbox. Using `--attach` would therefore mean a personal token, a `gh` binary in the template, a broker policy for the upload host, and a comment posted as the token’s owner rather than as `baymiai[bot]`. Recorded 2026-09-03 from upstream’s workflow comment, not tested against this App’s token. The one thing Blob cannot do that `--attach` can is an inline video player: GitHub only plays `<video>` from its own asset host.
...
**Vercel reports a preview through a commit _status_, which eve has no hook for.** Checked on 2026-09-04 against `PunGrumpy/website#2`: the deployment arrives as `POST /statuses` with `context: "Vercel"`, while the only check run the Vercel App writes is `Vercel Preview Comments`, which tracks its live-feedback widget rather than the deployment. `githubChannel` offers `onCheckSuite`, `onCheckRun` and `onWorkflowRun` but nothing for `status`, so there is no event to wait on for "the preview is ready". The pull request opened at 05:54:49 and the preview answered at 05:56:57, so `onPullRequest` on `opened` always races it. The capture turn therefore does not wait: it probes the preview the way `before-after` describes and falls back to a dev server in the sandbox, which needs no webhook at all.

**`agent-browser record` exists and the pinned version already has it.** `record start <path.webm> [url]` / `record stop` / `record restart` are in 0.35.1, the version `installAgentBrowser` puts in the template, so a video capture needs no upgrade. What is missing is the delivery: GitHub plays `<video>` only from its own asset host, which needs `gh --attach` and therefore a personal token (see above). A `.webm` on Blob is a link a reviewer downloads, which is worse than the still pair, so nothing captures video yet.
